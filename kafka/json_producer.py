...
def request_us_flights(access_key, retries=3, dep_airports=None, arr_airports=None, delay=5, **kwargs):
    if not dep_airports or not arr_airports:
        print("Please provide lists of departure and arrival airports.")
        return None

    flights = []
    for dep_iata in dep_airports:
        for arr_iata in arr_airports:
            if dep_iata != arr_iata:  # Skip flights within the same airport
                print(f"Fetching flights from {dep_iata} to {arr_iata}...")
                kwargs.update({"dep_iata": dep_iata, "arr_iata": arr_iata})
                response = request_data(access_key, retries=retries, delay=delay, **kwargs)
                if response and "data" in response:
                    flights.extend(response["data"])
                elif response is None:
                    print(f"No data for route {dep_iata} -> {arr_iata} or limit reached.")
                time.sleep(10)
